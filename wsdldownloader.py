...
def download_wsdl_imports(wsdl_url, wsdl_doc, output_dir):
    wsdl_import_elems = wsdl_doc.getElementsByTagName('wsdl:import')
    if wsdl_import_elems:
        print('Fetching child WSDLs for: ', get_wsdl_name(wsdl_url, wsdl_doc))
    for elem in wsdl_import_elems:
        ...
        download_wsdl_imports(child_wsdl_url, child_wsdl_doc, output_dir)
        download_xsd_imports(child_wsdl_url, child_wsdl_doc, output_dir)
...
def download_xsd_imports(wsdl_url, wsdl_doc, output_dir):
    xsd_import_elems = wsdl_doc.getElementsByTagName('xsd:import')
    if xsd_import_elems:
        print('Fetching XSD schemas for: ', get_wsdl_name(wsdl_url, wsdl_doc))
...
def get_wsdl_name(wsdl_url, wsdl_doc):
    ...
    if attrib and attrib.value:
        ...
        url_query_params = get_url_query_params_as_dict(wsdl_url)
        return url_query_params['wsdl']


def get_wsdl_file_name(wsdl_url, wsdl_doc):
    return get_wsdl_name(wsdl_url, wsdl_doc) + '.wsdl'


def process(wsdl_url, output_dir):
    ensure_dir_exists(output_dir)
    wsdl_doc = read_xml_from_url(wsdl_url)
    download_wsdl_imports(wsdl_url, wsdl_doc, output_dir)
    download_xsd_imports(wsdl_url, wsdl_doc, output_dir)
    wsdl_filename = get_wsdl_file_name(wsdl_url, wsdl_doc)
